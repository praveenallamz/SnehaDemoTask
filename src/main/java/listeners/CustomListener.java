...
public class CustomListener extends WebDriverFactory implements ITestListener, ISuiteListener, IInvokedMethodListener
{
    ...
    public void onFinish(ITestContext iTestContext) {
       ...
       // String htmlReportFile = System.getProperty("user.dir") +  ConfigReader.getValue("HtmlReportFullPath");
		...
		if (f.exists()) {
		try {
              Runtime.getRuntime().exec("C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe \"" + BaseMethod.HtmlReports+ "\"");
			} catch (IOException e) {
				 try {
					Runtime.getRuntime().exec("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe \"" + BaseMethod.HtmlReports+ "\"");
